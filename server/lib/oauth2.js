...
Strategy.prototype.userProfile = function(accessToken, done) {
  ...
  this._oauth2.get('https://api.fitbit.com/1/user/-/activities/heart/date/today/1d.json', accessToken, function (err, body, res) {
         if (err) { return done(new InternalOAuthError('failed to fetch user hearrate', err)); }
    try {
      var json = JSON.parse(body);
        
     var heartrate = { provider: 'fitbit' };
      heartrate.json = json ;
